...
class WPClickmeter {
    ...
    static function on_load() {
        ...
        try {
            ...
            if ($version == "1.2.5.7") {
                update_option('clickmeter_plugin_version', "1.2.6");
                $version = "1.2.6";
            }
            if ($version == "1.2.6") {
                WPClickmeter::update_tracking_link_table();
                WPClickmeter::update_tracking_pixel_table();
                update_option('clickmeter_plugin_version', "1.2.7");
                $version = "1.2.7";
            }
            if ($version == "1.2.7") {
                update_option('clickmeter_plugin_version', "1.2.8");
                $version = "1.2.8";
            }
    ...
    function update_tracking_link_table(){
        global $wpdb;
    ...
    static function campaignIsDeleted ($campaign){
        $api_key = WPClickmeter::get_option('clickmeter_api_key');
        $campaign_data = WPClickmeter::api_request('http://apiv2.clickmeter.com/groups/' . $campaign, 'GET', NULL, $api_key);
        return $campaign_data[deleted];
    }

    static function createCampaign ($type){
        $api_key = WPClickmeter::get_option('clickmeter_api_key');
        ...
        $body=array('name'=>$blog_name.$campaign_name);
    ...
    static function api_request($endpoint, $method, $body, $api_key, $from_js=false){
        try {
            $associative=true;
            $args = array(
                'method' => $method,
            ...
            if ($response['response']['code'] != 200) {
                ...
                $errors_body = json_decode($response['body'], $associative);

                // default errors
                $error_report = true;
                $error_message = "<div id='clickmeter-warning' class='error fade'>
                ...
                if ($response['response']['code'] == 401){
                    ...
                    if(!empty($errors_body) && !empty($errors_body[errors]) && is_array($errors_body[errors]) && !empty($errors_body[errors][0][code])){
                        if($errors_body[errors][0][code] == "0013"){
                            // destination url not compatible with hideUrl
                            $error[error_code] = 501;
                        }
                        else if ($errors_body[errors][0][code] == "0003"){
                            $error_report = false;
                            $error[error_code] = 503;
                            $error_message = "<div id='clickmeter-warning' class='error fade'>
					            <p style='color:black'><strong>ClickMeter: </strong>You exceeded the allowed number of datapoints.
					            </p>	
				            </div>";
                        }
                    }
    ...
    static function create_tracking_pixel($post_id, $tracking_pixel_id="", $update=false){
        ...
        if (WPClickmeter::campaignIsDeleted($group_id_TP)){
            $campaign_created = WPClickmeter::createCampaign(0);
            $group_id_TP = $campaign_created;
            WPClickmeter::store_option("clickmeter_TPcampaign_id", $campaign_created);
        }
...
function ajax_create_tl() {
    try {
        ...
        if (WPClickmeter::campaignIsDeleted($default_campaignId)){
            $campaignId = WPClickmeter::get_option("clickmeter_TLcampaign_id");
            if (WPClickmeter::campaignIsDeleted($campaignId)){
                $campaign_created = WPClickmeter::createCampaign(1);
                $default_campaignId = $campaign_created;
                WPClickmeter::store_option("clickmeter_TLcampaign_id", $campaign_created);
                WPClickmeter::store_option("clickmeter_default_campaignId_links", $campaign_created);
            }
            else {
                $default_campaignId = $campaignId;
                WPClickmeter::store_option("clickmeter_default_campaignId_links", $campaignId);
            }
        }
        ...
        $created_link = WPClickmeter::api_request('http://apiv2.clickmeter.com/datapoints', 'POST', json_encode($body), $api_key, true);

        if($created_link[id] != null) {
        ...
        else if($created_link[error_code]){
            status_header($created_link[error_code]);
            wp_die();
        }
        else {
            status_header(500);
